...
const saveSearchesCookies = (isFavorite, searchArr) => {
  ...
  for (let i = 0; i < searchArr.length; i++) {
    search +=
      encodeURI(searchArr[i].summoner) + (i < searchArr.length - 1 ? '%24' : '')
  }
...
export const saveItemsList = (list) => {
  var toSave = ''

  list.forEach((item) => {
    toSave += `${item.image.full}X`
  })

  Cookies.set('_itemsImgList', toSave)
}

export const getItemsList = () => {
  const itemsImgList = Cookies.get('_itemsImgList')
  let ret = []

  if (itemsImgList !== undefined && itemsImgList !== '') {
    const splited = itemsImgList.split('X')
    for (let i = 0; i < splited.length; i++) {
      if (splited[i].includes('.png'))
        ret.push({
          image: { full: splited[i] },
        })
    }
  }
